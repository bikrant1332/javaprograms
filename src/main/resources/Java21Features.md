...
Traditonal way for creating thread:
...
This simple example shows the difference in how threads are created: traditional threads require instantiating Thread and calling start(), while virtual threads use the JVM-managed method Thread.startVirtualThread() for lightweight thread creation
Summary:
	Traditional threads are OS threads with significant memory overhead and limited scalability.
	Virtual threads are lightweight, managed by the JVM, allowing millions of concurrent threads with low memory usage.
	Virtual threads improve resource efficiency by not blocking OS threads during I/O operations.
	Code-wise, virtual threads simplify concurrent programming with minimal changes to synchronous code.
...
Unnamed Classes in Java 21:
	Unnamed classes allow you to write Java programs without explicitly declaring a class name. The compiler implicitly wraps the top-level code in an
	unnamed class in the unnamed package. This simplifies the syntax for small programs or demos.
...
- Scoped Values (Preview)
	...
	Example:
	Here is an example of using Scoped Values in Java 21, illustrating how to create, bind, and access scoped values within a thread and its child threads using Structured Concurrency:
	
		import java.lang.ScopedValue;
		import java.util.concurrent.StructuredTaskScope;
		public class ScopedValueExample {
		    // Create a scoped value instance
		    private static final ScopedValue<String> CONTEXT = ScopedValue.newInstance();
		    public static void main(String[] args) {
		        // Bind the scoped value "TestValue" and run the code within this scope
		        ScopedValue.runWhere(CONTEXT, "TestValue", () -> {
		            System.out.println("In parent thread start, scoped value: " + CONTEXT.get());
		            try (var scope = new StructuredTaskScope<Void>()) {
		                // Fork child tasks that inherit the scoped value
		                scope.fork(() -> {
		                    System.out.println("In child thread 1, scoped value: " + CONTEXT.get());
		                    return null;
		                });
		                scope.fork(() -> {
		                    System.out.println("In child thread 2, scoped value: " + CONTEXT.get());
		                    return null;
		                });
		                scope.join(); // Wait for child tasks to finish
		            }
		            System.out.println("In parent thread end, scoped value: " + CONTEXT.get());
		        });
		        // Outside the scope, the scoped value is not bound
...
Explanation:
	...
	ScopedValue.runWhere(CONTEXT, "TestValue", () -> { ... }) binds the value "TestValue" to the scoped value CONTEXT for the duration of the lambda execution.
	Inside this scope, calling CONTEXT.get() returns the bound value.
	Using StructuredTaskScope and its fork() method, child virtual threads inherit the scoped value automatically.
	Outside the scope, the scoped value is unbound, so CONTEXT.isBound() returns false and CONTEXT.orElse(null) returns null.
...
Generational Heap Structure: 
	The heap is divided into young and old generations. New objects are allocated in the young generation, which is scanned frequently. Objects that survive longer are promoted to the old generation, which is scanned less often. This leverages the weak generational hypothesis that most objects die young, improving GC efficiency.
...
Efficient Large Object Handling: 
	Large objects can now be allocated in the young generation and either collected quickly if short-lived or promoted efficiently if long-lived, avoiding premature allocation in the old generation and improving memory usage.
Optimized Barriers and Remembered Sets: 
	It uses optimized store barriers and double-buffered remembered sets to track references between generations with minimal overhead, enhancing performance.
Region Density Analysis: 
	The collector analyzes the density of regions in the young generation to selectively evacuate those with many dead objects, reducing GC effort and improving responsiveness
